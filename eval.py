...
def test():
    ...
    for data, target, org_w, org_h in tqdm.tqdm(test_loader):
        data = data.to(device)
        output = model(data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', '-c',  type=str, 
        default='cfg/yolov3_kaist.cfg', help='network configuration file')
    parser.add_argument('--weights', '-w', type=str, nargs='+', 
        default=['weights/kaist_thermal_detector.weights'], help='initial weights file')
